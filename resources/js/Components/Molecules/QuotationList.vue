<template>
  <div class="space-y-2">
    ...
    <div class="overflow-auto space-y-2 lg:w-auto w-fit">
      <div
      ...
      <div v-if="filteredQuotationsResults.length > 0" class="space-y-2 overflow-auto">
        <QuotationListItem v-for="(document, index) in filteredQuotationsResults" :key="index" :document="document" :selected-all="selectedAll" :toggle-document-selection="toggleDocumentSelection"/>
      </div>
      <div v-else-if="searchQuotationResults.length > 0" class="space-y-2 overflow-auto">
        <QuotationListItem v-for="(document, index) in searchQuotationResults" :key="index" :document="document" :selected-all="selectedAll" :toggle-document-selection="toggleDocumentSelection"/>
      </div>
