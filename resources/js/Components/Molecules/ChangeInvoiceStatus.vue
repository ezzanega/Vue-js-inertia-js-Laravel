            ...
            <div class="px-4 sm:px-6">
                <div class="w-full py-3 flex flex-col space-y-2">
                    <div class="my-6 space-y-5">
                        <DefaultSelectInput name="status" label="Statuts" v-model="form.status" :options="InvoiceStatus"
...
const updateStatus = () => {
    // alert('Invoice status changé');
    form.put(route("6dem.invoice.updateStatus", { id: props.invoice.id, field: "status" }), {
        preserveScroll: true,
        preserveState: true,
        replace: true,
        onSuccess: () => {
